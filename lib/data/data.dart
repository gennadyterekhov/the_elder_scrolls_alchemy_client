...
class DataProvider {
  static Map<String, dynamic> _getMap() {
    return jsonDecode(_getFullJsonDataString());
  }

  static Map<String, Effect> getAllEffects() {
    final parsedJson = _getMap();
    Map<String, Effect> resultMap = {};

    parsedJson['effects']
        .forEach((id, effect) => resultMap[id] = Effect.fromJson(effect));

    return resultMap;
  }

  static Effect getEffectById(String id) {
    final parsedJson = _getMap();

    Effect effect = Effect.fromJson(parsedJson['effects'][id]);

    return effect;
  }

  static String _getFullJsonDataString() {
    const fullJsonDataString = '''
{
    "effects": {
        "000ae723": {
            "id": "000ae723",
